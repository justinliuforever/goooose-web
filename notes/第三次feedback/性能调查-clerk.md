...
| **Groq ASR** | 免费 30 RPM / Developer 300 RPM | ✗ → **必须升 Developer** |
| Trigger Hobby | 50 并发 run | ✓（用 ~8，不升套餐） |

Groq 是真天花板（XHS 全靠它）；DeepSeek 不是瓶颈。机器按秒计费（非月费）：medium-1x $0.000085/s、large-1x $0.00034/s —— 并行后跑得短，单次净更便宜。
...
| ASR | **XHS 直连 Groq**；YouTube 留 Deepgram 主 + Groq 兜底 | Deepgram 在 XHS 必败 |
| Groq 档位 | → **Developer（绑卡免费，300 RPM）** | XHS 全压 Groq；**用户侧操作** |
| Trigger 套餐 | Hobby **不变** | 50 并发对 4 用户够 |
| 自托管 | **不做** | 留到规模化再评 |

**预期：单次 Clerk(20) 48min → ~5–7min，$0.245 → ~$0.10（更快又更便宜）。**

实施落点：`analyze-channel.ts`（:328 并发 / :332 机器 / :476-686 XHS 并行 / :576 Flash+tokens / :268 parseAnalysis / task queue）、`monitor-competitors.ts`（queue）、`packages/shared/src/clients/asr.ts`（XHS 直连 Groq）。
