...
**进度**：Beta v0.7 已上线（2026-07-15，goooose.com）。历轮：三轮 feedback（IA 三层重构、Clerk 拆对标解耦、防编造、ASR 转写修复、进度条/ETA、UIUX motion 层）→ R4 多项目 + SOP map-reduce → v0.5 内测准入 + 分钟配额 + 管理后台 → v0.6 更名搬砖小鹅 Goooose + 锚点化圣经 + 圣经文件导入 → v0.7 SOP 文档阅读器 + 深浅色 + 小红书分享链/笔记回跳 + Clerk 拆解深化 + 生成质量打磨。版本 release notes 见 `notes/releases/`；各轮执行文档见 `notes/archive/`。
...
- 视频列表按平台自适应；点标题回跳原帖（XHS 经 `/api/xhs` 懒解析现取 `xsec_token`，永不过期）；页头「xxx 的账号画像」；分析完自动刷新
- 单视频详情：13 个分析维度 + transcript + 「单条爆款拆解」（v0.7 补齐脚本结构蓝图 / 讲故事框架 / 留人机制三节，与全账号 SOP 对齐）
- 三种 SOP（map-reduce 生成：每视频 Flash 小结 → 分批 Pro 归并 → Pro 合成）+ 文档阅读器渲染（章节锚点、表格卡片、引用 callout、左侧 sticky 章节导航）：
  - **human**：把频道选题方法论用人话讲清楚
  - **ai_reference**：AI 结构化 SOP，给 Muse/Poet prompt 注入
  - **hottest**：仅基于 Top-N 高互动视频拆爆款机制（避免在低互动样本上幻觉）
  - SOP 卡片标注源帖子标题；小红书内容标「帖子N（图文 / 视频）」而非「视频N」
